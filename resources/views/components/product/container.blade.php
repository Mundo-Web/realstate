<div x-data="{ showAmbiente: false }" @mouseenter="showAmbiente = true" @mouseleave="showAmbiente = false"
  ...
  <div class="{{ $bgcolor }} product_container flex flex-col justify-center relative bg-[#191919]">
      <a href="{{ route('producto', $item->id) }}" class="flex flex-col">
  ...
  <div href="{{ route('producto', $item->id) }}" class="p-3 lg:p-6 flex flex-col gap-2 bg-[#191919] rounded-b-xl">
    ...
    <div class="text-sm xl:text-base 2xl:text-lg text-left overflow-hidden font-PlusJakartaSans_Regular text-white cortartexto">
      {{ mb_strimwidth($item->extract, 0, 95, '...') }} Una impresionante villa de 4 dormitorios y 3 baños en un tranquilo vecindario suburbano... Leer más
    </div> 
    ...
    <div class="flex flex-row justify-between items-end w-full gap-3">
      <div class="flex flex-col gap-1 text-white font-PlusJakartaSans_Regular">
          <span class="text-sm 2xl:text-lg">Precio</span>
          <span class="text-lg lg:text-xl xl:text-2xl 2xl:text-3xl font-PlusJakartaSans_Bold">$4,000,000</span>
      </div>
